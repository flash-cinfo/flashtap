...
void Engine::create(int screenWidth, int screenHeight, const std::string &name,
                    bool verticalSync, sf::Color background, sf::Uint32 style) {
  _name = name;
  screen->open(screenWidth, screenHeight, name, verticalSync, background,
               style);

  Log::log("Engine::create(): started engine (" + std::to_string(screenWidth) +
           "x" + std::to_string(screenHeight) + ") with title '" + name + "'.");
  Time::update();
  ...
  while (screen->isOpen()) {
    ...
    // While printing debug info we will take into account only timer names
    // witch start with 'd '
      ...
      if (_useOpenGL) {
        ...
        for (auto &it : *world) {
          if (it.second->isVisible()) {
            ...
            screen->glDrawMesh(geometry, view, model,
                               3 * it.second->triangles().size());
            delete[] model;
          }
        }
        screen->pushGLStates();
...
void Engine::exit() {
  ...
  Log::log("Engine::exit(): exit engine (" + std::to_string(screen->width()) +
           "x" + std::to_string(screen->height()) + ") with title '" +
           screen->title() + "'.");
}
...
void Engine::printDebugInfo() const {
  ...
  if (_showDebugInfo) {
    ...
    std::string text =
        ...
        "\n RY:" + std::to_string(camera->angle().y() / Consts::PI) +
        "PI\n RL: " +
        std::to_string(camera->angleLeftUpLookAt().x() / Consts::PI) +
        "PI\n\n" + std::to_string(screen->width()) + "x" +
        std::to_string(screen->height()) + "\t" + std::to_string(Time::fps()) +
        " fps";
    ...
    for (auto &[timerName, timer] : Time::timers()) {
      ...
      screen->drawTetragon(
          Vec2D{xPos, yPos + height * i},
          Vec2D{xPos + width, yPos + height * i},
          Vec2D{xPos + width, yPos + height + height * i},
          Vec2D{xPos, yPos + height + height * i},
          {static_cast<sf::Uint8>(255.0 * static_cast<double>(width) /
                                  timerWidth),
           static_cast<sf::Uint8>(
               255.0 * (1.0 - static_cast<double>(width) / timerWidth)),
           0, 100});
      ...
      screen->drawText(
          timerName.substr(2, timerName.size()) + ":\t" +
              ...
              "%)",
          Vec2D{xPos + 10, yPos + height * i + 5}, 30, sf::Color(0, 0, 0, 150));
    ...
    screen->drawTetragon(
        Vec2D{xPos, yPos + height * i}, Vec2D{xPos + width, yPos + height * i},
        Vec2D{xPos + width, yPos + height + height * i},
        Vec2D{xPos, yPos + height + height * i},
        {static_cast<sf::Uint8>(255.0 * static_cast<double>(width) /
                                timerWidth),
    ...
    screen->drawText(
        "other:\t" + std::to_string(1000 * (totalTime - timeSum)) + " ms \t (" +
